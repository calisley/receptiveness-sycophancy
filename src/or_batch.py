...
class BatchUnsupportedError(RuntimeError):
    """OpenRouter reports this model has no Batch API endpoint."""


def _is_batch_unsupported(body: str) -> bool:
    b = body.lower()
    return "does not have a :batch endpoint" in b or "does not have a batch endpoint" in b
...
def _headers() -> dict[str, str]:
    ...
    return {
        ...
        "HTTP-Referer": "https://github.com/calisley/receptiveness-sycophancy",
        "X-Title": "receptiveness-sycophancy",
    }
...
def _http_json(method: str, url: str, payload: dict | None = None, timeout: float = 120.0) -> dict:
    ...
    for attempt in range(8):
        ...
        try:
            ...
            if e.code == 400 and _is_batch_unsupported(body):
                raise BatchUnsupportedError(body[:500]) from e
