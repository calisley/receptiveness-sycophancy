...
Positive dims: hedging, emphasize_agreement, acknowledge_perspective,
reframe_positive, invite_curiosity.
Negative dims: negation, adverb_limiter, disagreement, negative_emotion,
...
JUDGE_VERSION = "receptiveness_hear_likert0_signed"
...
# score_receptiveness_likert0_signed requires system= from the caller.


class HearLikert0SignedJudgment(BaseModel):
...
def positive_sum(j: HearLikert0SignedJudgment) -> int:
    return sum(int(getattr(j, k)) for k in POSITIVE_FEAT)


def positive_mean(j: HearLikert0SignedJudgment) -> float:
    return positive_sum(j) / float(len(POSITIVE_FEAT))


def negative_sum(j: HearLikert0SignedJudgment) -> int:
    return sum(int(getattr(j, k)) for k in NEGATIVE_FEAT)


def negative_mean(j: HearLikert0SignedJudgment) -> float:
    return negative_sum(j) / float(len(NEGATIVE_FEAT))


def hear_sum(j: HearLikert0SignedJudgment) -> int:
    """Sum of the four core H.E.A.R. positives (H/E/A/R)."""
    return (
        int(j.hedging)
...
def hear_mean(j: HearLikert0SignedJudgment) -> float:
    return hear_sum(j) / 4.0


def judgment_to_row(j: HearLikert0SignedJudgment, **extra) -> dict:
    row = {
...
async def score_receptiveness_likert0_signed(
    client,
    model: str,
    question: str | None,
    response: str,
    ...
    *,
    ...
    kwargs: dict = {
        ...
        "response_format": HearLikert0SignedJudgment,
    ...
    for attempt in range(retries):
        try:
            ...
            schema_chars = len(_json.dumps(HearLikert0SignedJudgment.model_json_schema()))
            est = estimate_tokens(
                system + "\n" + user,
