...
Requires **Python 3.12+**, a virtual environment, and API keys. Put them in a `.env` file in the repo root:
...
LLM outputs will not match the paper cell-for-cell. After recompiling CSVs, rerun `Rscript analysis.R`.
...
Generate first- and third-person replies (with verdicts).

```bash
python scripts/gen_aita.py --model all --arm 1p3p \
  --judge-verdicts --out data/mitigation/gens.jsonl \
...
To generate the prompt mitigation results, use `--arm prompt` instead of `--arm 1p3p`.
